...
export function isOver20(dateOfBirth: Date, referenceDate: Date = new Date()): boolean {
  return calculateAge(dateOfBirth, referenceDate) >= MIN_AGE_REQUIREMENT;
}

/** @deprecated Use isOver20 instead */
export const isOver18 = isOver20;
...
export function getMinAgeDateOfBirth(referenceDate: Date = new Date()): Date {
  const dob = new Date(referenceDate);
  dob.setFullYear(dob.getFullYear() - MIN_AGE_REQUIREMENT);
  return dob;
}

/** @deprecated Use getMinAgeDateOfBirth instead */
export const getDateOfBirth18YearsAgo = getMinAgeDateOfBirth;
