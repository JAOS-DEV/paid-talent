...
describe("age-verification helpers", () => {
  ...
  describe("isOver20", () => {
    it("should return true for someone exactly 20 years old", () => {
      ...
      expect(isOver20(dob, reference)).toBe(true);
    });
    ...
    it("should return false for someone under 20", () => {
      ...
      expect(isOver20(dob, reference)).toBe(false);
    });
  ...
  describe("isOver18 (deprecated alias)", () => {
    it("should be an alias for isOver20", () => {
      expect(isOver18).toBe(isOver20);
    });
  });
  ...
  describe("getMinAgeDateOfBirth", () => {
    it("should return date 20 years before reference", () => {
      const reference = new Date("2024-06-15");
      const result = getMinAgeDateOfBirth(reference);
    ...
    it("should handle leap year (Feb 29 becomes Mar 1 in non-leap year)", () => {
      ...
      const result = getMinAgeDateOfBirth(reference);
      expect(result.getFullYear()).toBe(2004);
      expect(result.getMonth()).toBe(1); // Feb in 2004 (leap year) stays Feb 29
      expect(result.getDate()).toBe(29);
    });
  });

  describe("getDateOfBirth18YearsAgo (deprecated alias)", () => {
    it("should be an alias for getMinAgeDateOfBirth", () => {
      expect(getDateOfBirth18YearsAgo).toBe(getMinAgeDateOfBirth);
    });
  });
